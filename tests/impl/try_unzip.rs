...
fn try_unzip_fail_a() {
    ...
    let err = err.left().expect("Should be left");

    assert_eq!(err.error.item, 1, "Should be colliding value");
}
...
fn try_unzip_fail_b() {
    ...
    let err = err.right().expect("Should be right");

    assert_eq!(err.error.item, 2, "Should be colliding value");
}
...
fn try_unzip_recover_partial_b_on_a_failure() {
    ...
    assert_eq!(err.error.item, 1);
    assert_eq!(err.failed, HashSet::from([1, 2])); // Failed collection A
    assert_eq!(err.partial, HashSet::from([10, 20])); // Successful collection B
...
fn try_unzip_recover_partial_a_on_b_failure() {
    ...
    assert_eq!(err.error.item, 2);
    assert_eq!(err.failed, HashSet::from([1, 2])); // Failed collection B
    assert_eq!(err.partial, HashSet::from([10, 20, 30])); // Successful collection A
    assert_eq!(err.remaining.len(), 1);
}
