...
test_format!(
    display_format_err,
    ResultCollectError::new(TestError::<()>::new("iter error"), Err::<(), _>(TestError::<()>::new("collection error")), 0..0),
    "{}",
...
test_source!(
    error_trait_source,
    ResultCollectError::new(TestError::<()>::new("iter error"), Ok::<_, TestError::<()>>(HashSet::from([1, 2, 3])), 0..0),
    TestError::<()>
);
